...
public class AutoMerger implements RequestHandler<S3Event, Void> {
    ...
    public Void handleRequest(S3Event s3event, Context context) {
        ...
        String prefix_regex = "/(January|February|March|April|May|June|July|August|September|October|November|December)/\\d{1,2}/(morning|evening)/";
        ...
        for (S3Event.S3EventNotificationRecord record : s3event.getRecords()) {
            ...
            //remembrance/July/1/morning/
            String moment = matcher.group(2);
            String month = matcher.group(1);
            String audio_merge_bucket = System.getenv("audio_merge_bucket");
            String audio_merge_object_key = AWSUtil.searchS3ObjectKey(audio_merge_bucket, StringUtils.remove(StringUtils.substring(objectKey, 0, StringUtils.lastIndexOf(objectKey, "/")), moment), StringUtils.capitalize(moment)+".audioMerge") ;
    ...
    public static File mergeAudio(Map<String, String> tags) {
        ...
        File local_audio_directory = new File("/tmp");
        if( !local_audio_directory.exists() ){
            local_audio_directory.mkdirs();
        }

        File tempRemFolder = new File("/tmp/remembrance");
        if( !tempRemFolder.exists() ){
            tempRemFolder.mkdirs();
        }

        String audioKey = tags.get("audio_key");
        String audioFormat = tags.get("audio_merged_format");
        String publish_date = StringUtils.replace(tags.get("publish_date"), " ", "/");
        String rem_path = local_audio_directory.getAbsolutePath()+"/"+tags.get("output_prefix")+publish_date+"/"+StringUtils.lowerCase(tags.get("timing"));
        File rem_directory = new File( rem_path );
        if( !rem_directory.exists() ){
            rem_directory.mkdirs();
        }
        
        AWSUtil.copyS3Objects( tags.get("output_bucket"),
                tags.get("output_prefix")+publish_date+"/"+StringUtils.lowerCase(tags.get("timing"))+"/",
                tags.get("output_format"),
                local_audio_directory.getAbsolutePath()+"/");

        List<File> toBeMerged = new ArrayList<>();
        toBeMerged.addAll(List.of(rem_directory.listFiles(new FilenameFilter() {
            @Override
        ...
        String finalName = tags.get("name_prefix")+"_"+StringUtils.replace(tags.get("publish_date"), " ", "_")+"_"+tags.get("timing")+"."+tags.get("audio_merged_format");
